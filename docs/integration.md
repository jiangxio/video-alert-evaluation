...
文件名必须编码「告警类型 ID」与「触发时间戳」，格式：

```
{前缀}_{unix时间戳}_{告警类型ID}.png
```

示例：`402_1774925112_103.png` 表示告警类型 103、时间戳 1774925112。

解析规则（见 `app/services/verification_service.py`）：
- 优先匹配 `[_\-](\d+)\.[^.]+$`，取末尾数字段为告警类型 ID
- 回退匹配 `(\d+)\.[^.]+$`

### 批量导入

通过「告警数据集」页面创建数据集，支持从 ZIP 批量导入告警图片。系统从文件名自动识别事件类型。
