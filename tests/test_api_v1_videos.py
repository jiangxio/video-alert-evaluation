...
def test_upload_then_list(client):
    ...
    assert body["code"] == 0
    vid = body["data"]["id"]
    assert body["data"]["filename"] == "0460000001_test.mp4"
    assert body["data"]["video_id"] == "0460000001"
    assert resp.headers["Location"].endswith(f"/api/v1/videos/{vid}")
...
def test_get_single_and_not_found(client):
    ...
    assert not_found.get_json()["code"] == 20120
...
def test_patch_video_id(client):
    ...
    assert bad.get_json()["code"] == 10108
...
def test_eval_sets_crud(client):
    ...
    assert resp.headers["Location"].endswith(f"/api/v1/videos/eval-sets/{sid}")
